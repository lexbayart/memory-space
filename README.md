...
## ⌨️ Быстрая инструкция

### Навигация
- **Колёсико мыши / свайп по горизонтали** — перемещение по временной линии
- **Перетаскивание шарика** на шкале внизу — ручной переход к любой дате
- **Клик по узлу** — открыть карточку заметки
- **Клик по связи** в карточке — перейти к связанной заметке

### Горячие клавиши
| Клавиша | Действие |
|---------|----------|
| `T` | Переключить тему (тёмная ↔ светлая) |
| `D` | Показать/скрыть панель отладки |
| `K` | Включить/выключить режим отладки шкалы |
| `Escape` | Закрыть карточку / сбросить поиск |

### Поиск и фильтры
- **Клик по 🔎** (сверху по центру) — открыть строку поиска
- **Двойной клик по полю поиска** — сбросить все фильтры и хэштеги
- **Клик по хэштегу** в боковых колонках — отфильтровать заметки
- **Клик по бейджу хэштега** в строке поиска — убрать этот фильтр

### Временная шкала
- **Клик по зелёному ромбу** (якорь «сейчас») — заморозить время
- **Клик по белому квадрату** (заморожен) — разморозить и вернуться в настоящее
- **Клик по треугольнику-якорю** на линиях — перейти к этой дате

### Редактирование заметок
- **Кнопка редактирования** в карточке — режим правки
- **Клик по эмодзи** — выбрать другой эмодзи
- **Клик по цветному кружку** — палитра цветов
- **Перетаскивание даты** — изменить дату заметки
- **Кнопка «Сохранить»** — применить изменения
